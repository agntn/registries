...
| `fetchMaintainers` | `GET /pypi/{name}/json`, `author`, `author_email`, `maintainer` and `maintainer_email` |
...
Authors first with `role: "author"`, then maintainers with `role: "maintainer"`, deduplicated by email. PEP 621 projects leave `author` null and list everyone in `author_email` as `Name <email>, Name <email>`, so pydantic comes back as twelve entries and requests as one author plus two maintainers. A plain `author` name pairs with a bare `author_email`; a name that has no address of its own stays one entry beside the emailed people. RFC 822 comments and quoted display names are read too. The project's owners on pypi.org are not read.
...
- `licenses` is the PEP 639 `license_expression` when the project ships one. Older projects only have the free text `license` field, which goes through `normalizeLicense`, and a long license body is returned as is.
