...
- crates.io requires a `User-Agent` and rate limits anonymous clients. The default client sends `agntn-registries/<version> (+https://github.com/agntn/registries)` with the installed version; set your own through `new Client({ userAgent })` if you make many calls.
