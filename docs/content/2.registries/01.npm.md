...
Maintainers come first with an empty `role`, then the latest version's `author` with `role: "author"` and its `contributors` with `role: "contributor"`, deduplicated by name and email. A maintainer is an npm account, so `login` and `name` are both its username. The author and the contributors are the people named in `package.json`, with whatever `name`, `email` and `url` npm has and no `login`, because npm knows no account for them.
