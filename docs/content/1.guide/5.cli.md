...
`--json` prints what the library returns, so a script that reads it gets the `Package`, `Version[]`, `Dependency[]` or `Maintainer[]` shape documented under [Lookups](/guide/lookups). `versions` is the one exception: its JSON is the twenty newest, or as many as `--limit` says, not the whole list `fetchVersions` hands back.
