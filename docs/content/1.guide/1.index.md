...
Importing `@agntn/registries` runs nothing. The six built-in adapters sit in a manifest, and `create()` imports one the first time a PURL of its type comes through, so any PURL whose type is `npm`, `cargo`, `pypi`, `gem`, `composer` or `alpm` resolves without a switch statement anywhere and without parsing the other five.
